...
class HotTopicsViewSet(ViewSet):
    ...
    def atlas(self, request: Request):
        ...
        max_count_topic_node_id = TopicModel.objects.filter(task=task_info).order_by('-document_count').values_list('id',flat=True).first()
        similarity_data = [
            {
    ...
    def river(self,request:Request):
        ...
        results.update({
            ...
            'data':[
                ...
                for year,count,topic__name in TopicYearCountResult.objects.filter(topic_id__in=ids).values_list('year', 'count', 'topic__name')
            ]
        })
        return Response(results)
    ...
    def front_topic_list(self, request):
        ...
        return Response({
            'data':[
                ...
                TopicModel.objects.filter(task_id=task_info.taskid).order_by('sort').values_list('id', 'name', 'sort','keywords', 'document_count')
            ]
        })
...
class TopicViewSet(ViewSet):
    ...
    def topic_list(self, request):
        ...
        return Response({
            'data':[
                ...
                TopicModel.objects.filter(task_id=task_info.taskid).order_by('sort').values('id', 'name', 'sort','keywords', 'task_id')
            ]
        })
    ...
    def posts(self, request):
        task_info: TaskInfo = request.task_info
        count_top_n_keywords__data = ClusterTask.load_func_data('count_top_n_keywords',task_instance=task_info,model_name=ClusterTask.__name__)
...
class EmergingTechTopicViewSet(ViewSet):
    ...
    def topic_list(self,request):
        ...
        return Response({
            'data':[
                ...
                for it in EmergingTechTopicModel.objects.filter(task_id=task_info.taskid).order_by('sort').values('topic_id','name','sort')
            ]
        })
    ...
    def topic_update(self,request):
        task_info: TaskInfo = request.task_info
        for it in request.ARGS['bulk']:
            em_tech_topic = EmergingTechTopicModel.objects.filter(task_id=task_info.taskid,sort=int(it['sort'])).first()
    ...
    def burst(self, request):
        task_info: TaskInfo = request.task_info
        document_tokenizer_combine__data = ClusterTask.load_func_data('document_tokenizer_combine',task_instance=task_info,model_name=ClusterTask.__name__)
        return Response({
            'year_range':[document_tokenizer_combine__data['year_range'][0],document_tokenizer_combine__data['year_range'][1]-1],
            'data':BurstWordsResults.objects.filter(task=task_info,end__gte=F('start')).order_by('-weight').annotate(length=(F('end')-F('start')+1)).values('word','start','end','length','weight')
        })
...
class RDTENViewSet(ViewSet):
        ...
        return Response({
            'data':[
                ...
                RDTENProjectModel.objects
                    ...
                    .annotate(next_five_years=F('total_budget') + F('budget_fy1') + F('budget_fy2') + F('budget_fy3') + F('budget_fy4'))
                    .annotate(program_name=F('program__name'))
                    .values('program_code', 'program_name', 'id', 'name', 'fiscal_year', 'total_budget','next_five_years')
            ]
        })
    ...
    def by_topic(self, request):
        ...
        return Response({
            'data':list(
                RDTENByTopicModel.objects.filter(ts=task_info).annotate(topic_name=F('topic__name')).values('topic_id','topic_name','year','count','money')
            )
        })
    ...
    def by_rdten(self, request):
        ...
        return Response({
            'data':list(
                RDTENByRDTENModel.objects.filter(ts=task_info).annotate(rdten_name=F('rdten__document__name')).values('rdten_id','rdten_name','year','count','money')
            )
        })
